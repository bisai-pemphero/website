...
public partial class StudentInvoiceCreate : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string sql = @"SELECT COUNT(*) FROM Fees WHERE 
            studentId = @studentId AND TermId = @TermId 
            AND SchoolId = @SchoolId AND fees_Category = @feesCategory";

            using (SqlCommand cmd = new SqlCommand(sql, appconSQL2))
            {
                cmd.Parameters.Add("@studentId", SqlDbType.VarChar).Value = lblStudentId.Text.Trim();
                cmd.Parameters.Add("@TermId", SqlDbType.VarChar).Value = termId;
                cmd.Parameters.Add("@feesCategory", SqlDbType.VarChar).Value = categoryId;
                ...
                if (count > 0)
                {
                    lblError.Text = "This invoice already exists, try searching in Invoices";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowToast",
                     "showErrorToast('" + lblError.Text.Replace("'", "\\'") + "');", true);
                    return;
                }
        ...
        [FeesStatus], [InitalPaymentDate], [amountPaid], [balance], 
        [registeredBy],[dateRegistered])
	    VALUES (@feesCategory, @TermId, @studentId, @SchoolId, @TotalFees,
