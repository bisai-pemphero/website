...
public partial class StudentInvoice : System.Web.UI.Page
{
  ...
  Join FeesCategory as FC on FC.CategoryId = F.fees_Category
    where F.balance > 0 and F.SchoolId = @schoolId and CAST(GETDATE() AS DATE) BETWEEN ST.Startdate AND ST.Enddate", connection);
    ...
    {
        Response.Redirect("StudentInvoiceDelete.aspx");
    }
    ...
    {
        Response.Redirect("StudentInvoiceOld.aspx");
    }
}