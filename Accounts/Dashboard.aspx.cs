    ...
    private void LoadCharts()
    {
        using (SqlConnection con = new SqlConnection(connStr))
        {
            ...
            SqlCommand cmd = new SqlCommand(@"
                ...
                WHERE t.IsActive= 'True' AND f.SchoolId=@schoolId1
                GROUP BY c.ClassName ORDER BY c.ClassName", con);
            cmd.Parameters.AddWithValue("@schoolId1", lblSchoolId.Text.Trim());
            SqlDataReader dr = cmd.ExecuteReader();
            ...
            cmd.CommandText = @"
                ...
                INNER JOIN SchoolTerm t ON f.TermId=t.TermId
                WHERE t.IsActive='True' AND f.SchoolId= @schoolId2
