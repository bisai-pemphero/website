...
public partial class ViewStudent : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("RecoverStudent.aspx");
    }
    ...
    public void LoadStudents()
    {
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            SqlCommand command = new SqlCommand(@"Select S.StudentID, S.[FirstName], 
                S.[Middlename], S.[LastName], S.[Gender], C.[ClassName]  from Students as S
                Join Classes as C on C.ClassID = S.[CurrentClassID] 
	            where S.[IsDeleted] = 0 and S.Status = 'Active' 
                And S.SchoolId = @schoolId", connection);
            ...
            while (reader.Read())
            {
                ...
                Response.Write("<td>" + reader["Gender"] + "</td>");
                Response.Write("<td>" + reader["ClassName"] + "</td>");
                Response.Write("<td>");

                Response.Write("<a href='EditStudent.aspx?studentId=" + Id + "' class='btn'>");
                Response.Write("<i class='fas fa-edit'></i></a>");
                Response.Write("</td>");
                Response.Write("<td>");
                Response.Write("<a href='StudentDetails.aspx?studentId=" + Id + "' class='btn'>");
    ...
    {
        Response.Redirect("ChangeClass.aspx");
    }
}