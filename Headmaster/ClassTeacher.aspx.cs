...
public partial class UsersView : System.Web.UI.Page
{
    ...
    public void LoadStaff()
    {
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            SqlCommand command = new SqlCommand(@"Select FT.[FormTeacherId], FT.Teachername, c.ClassName  from  [FormTeachers] as FT
	                                            Join Classes as C ON C.ClassID = FT.[ClassId]
	                                            where FT.[SchoolId] = @schoolId order by FT.Teachername ", connection);
            command.Parameters.AddWithValue("@schoolId", lblSchoolId.Text.Trim());
            SqlDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                string Id = reader["FormTeacherId"].ToString();

                Response.Write("<tr>");
                Response.Write("<td>" + reader["Teachername"] + "</td>");
                Response.Write("<td>" + reader["ClassName"] + "</td>");
                Response.Write("<td>");

                Response.Write("<a href='ClassTeacherEdit.aspx?userId=" + Id + "' class='btn btn-warning'>");
                Response.Write("<i class='fas fa-edit'> Edit</i></a>");
                Response.Write("</td>");
                Response.Write("</tr>");
            }
    ...
    {
        Response.Redirect("ClassTeacherAdd.aspx");
    }
}