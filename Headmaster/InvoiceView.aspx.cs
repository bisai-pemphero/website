...
public partial class AddSchool : System.Web.UI.Page
{
    ...
    private void BindInvoiceGrid()
    {
        try
        {
            ...
            {
                string query = @"Select InvoiceId, InvoiceNo, Description, Total from SchoolInvoice
    ...
    protected void gvSchools_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            ...
            Response.Redirect("InvoiceEdit.aspx?Id=" + invoiceId);
        }
        if (e.CommandName == "View")
        {
            string invoiceId = e.CommandArgument.ToString();
            Response.Redirect("InvoiceAllDetail.aspx?Id=" + invoiceId);
        }
        if (e.CommandName == "Payment")
        {
            string invoiceId = e.CommandArgument.ToString();
            Response.Redirect("Pay.aspx?Id=" + invoiceId);
        }
    }
    ...
    protected void gvSchools_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            string invoiceId = gvSchools.DataKeys[e.RowIndex].Value.ToString();
          
            // Now delete the database record
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string deleteQuery = @"Delete from SchoolInvoice where InvoiceId = @invoiceNo";
                SqlCommand deleteCmd = new SqlCommand(deleteQuery, con);
                deleteCmd.Parameters.AddWithValue("@invoiceNo", invoiceId);

                con.Open();
                int result = deleteCmd.ExecuteNonQuery();
              

                string deletePayment = @"Delete from SchoolPayments where InvoiceId = @invoiceNo";
                SqlCommand PaymentCmd = new SqlCommand(deletePayment, con);
                PaymentCmd.Parameters.AddWithValue("@invoiceNo", invoiceId);

              
                int detail = PaymentCmd.ExecuteNonQuery();
                con.Close();
        ...
        {
            lblError.Text = "Error deleting Invoice: " + ex.Message;
    ...
    private void BindInvoiceGrid(string searchTerm = "")
    {
        try
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                {
                    query += "WHERE Description LIKE @SearchTerm OR Total LIKE @SearchTerm OR InvoiceNo LIKE @SearchTerm";
                }
    ...
    {
        BindInvoiceGrid(txtSearch.Text.Trim());
    }
