...
public partial class AddSchool : System.Web.UI.Page
{
    ...
    protected void drpClass_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            string[] className = drpClass.Text.Split('|');
            int classId = int.Parse(className[0]);

            drpSubject.Items.Clear();
            drpSubject.Items.Add("");

            string sql = @"Select SubjectId, SubjectName from Subjects where [ClassId] = @classId 
            and TeacherId = @teacherId";
            SqlCommand cmd = new SqlCommand(sql, appconSQL2);
            cmd.Parameters.AddWithValue("@classId", classId);
            cmd.Parameters.AddWithValue("@teacherId", lblTeacherId.Text.Trim());
           

            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                drpSubject.Items.Add(dr.GetInt32(0) + "|" + dr.GetString(1));
            }
            lblError.Text = "";
        ...
        catch (Exception ex)
        {
            ...
            lblError.Text = "Load teacher subject error" + ex;
        }
        finally
        {
            //
    ...
    protected void btnNext_Click(object sender, EventArgs e)
    {
        ...
        string[] subjet = drpSubject.Text.Split ('|');
        int subjectId = int.Parse(subjet[0]);
        ...
        Response.Redirect("ExamMark.aspx");
    }
}