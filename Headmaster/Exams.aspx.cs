...
public partial class UsersView : System.Web.UI.Page
{
    ...
    public void LoadExams()
    {
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            SqlCommand command = new SqlCommand(@" Select E.[ExamsId], E.[Exam_name], E.[Exam_start], E.[Exam_End],
            CONCAT(DATENAME(YEAR, A.[Start_year]), '-', YEAR(A.[End_year])) as AcademicYear, T.TermName 
            from Exams as E Join Academic_Year as A on E.AcademicYear = A.AcademicyearId 
            Join SchoolTerm as T on E.TermId = T.TermId
            where E.SchoolId = @schoolId", connection);
            command.Parameters.AddWithValue("@schoolId", lblSchoolId.Text.Trim());
            SqlDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                string Id = reader["ExamsId"].ToString();

                Response.Write("<tr>");
                Response.Write("<td>" + reader["Exam_name"] + "</td>");
                Response.Write("<td>" + reader["Exam_start"] + "</td>");
                Response.Write("<td>" + reader["Exam_End"] + "</td>");
                Response.Write("<td>" + reader["AcademicYear"] + "</td>");
                Response.Write("<td>" + reader["TermName"] + "</td>");
                Response.Write("<td>");
                Response.Write("<a href='ExamEdit.aspx?id=" + Id + "' >");
