...
public partial class AddSchool : System.Web.UI.Page
{
    ...
    public void LoadGrade(string gradeId)
    {
        try
        {
            string sql = @" Select [Grade], [Minimum_Mark], [Maxmum_Mark], 
            [Remark], [Level] from GradingSystem where Id = @Id 
            ...
            if (dr.Read())
            {
                ...
                txtMinmumMark.Text = dr["Minimum_Mark"].ToString();
                txtMaxmumMark.Text = dr["Maxmum_Mark"].ToString();
                txtRemark.Text = dr["Remark"].ToString();
                drpLevel.Text = dr["Level"].ToString();
            }
            else
            {
                lblError.Text = "Grade not found or you don't have permission to access it.";
        ...
        {
            lblError.Text = "Loading grade error: " + ex.Message;
            ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowToast", "showErrorToast('" + lblError.Text + "');", true);
        }
        finally
        {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        {
                //update exams
                string sql = @"  Update [GradingSystem] set [Grade] = @grade, [Minimum_Mark] = @min,
                [Maxmum_Mark] = @max, [Remark] = @remark, [Level] = @level
                ...
                cmd.Parameters.AddWithValue("@schoolId", lblSchoolId.Text.Trim());
            cmd.Parameters.AddWithValue("@Id", lblGradeId.Text.Trim());
            cmd.ExecuteNonQuery();
                cmd.Dispose();
                              
                Reset();
                ScriptManager.RegisterStartupScript(this, GetType(), "ShowSuccessAlert", "showAlert('success', 'Grade Updated Successfully!')", true);
        }
        catch (Exception ex)
        {
    ...
    protected void btnEdit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string sql = @"  Delete from [GradingSystem] where [Id] = @Id and [SchoolId] =@schoolId";
            ...
            {
                Reset();
                ScriptManager.RegisterStartupScript(this, GetType(), "ShowSuccessAlert", "showAlert('success', 'Grade Deleted Successfully!')", true);
            }
            else
            {
                lblError.Text = "Failed to Delete Grade. Please try again.";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowToast", "showErrorToast('" + lblError.Text + "');", true);
            }
        }
        catch (Exception ex)
