...
public partial class Payment : System.Web.UI.Page
{
    ...
    private void BindInvoiceGrid()
    {
        try
        {
            ...
            {
                string query = @"Select SP.[PaymentId], SI.InvoiceNo, SP.[Amount_Paid], 
                SP.[PaymentMode], SP.[Reference], SP.[PaymentDate] from SchoolPayments as SP
                Join SchoolInvoice as SI on SI.InvoiceId = SP.InvoiceId where SI.SchoolId = @schoolId";
        ...
        {
            lblError.Text = "Error loading payments: " + ex.Message;
    ...
    protected void gvSchools_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            ...
            Response.Redirect("PayEdit.aspx?Id=" + paymentId);
        }
    ...
    private void BindInvoiceGrid(string searchTerm = "")
    {
        try
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                {
                    query += "WHERE InvoiceNo LIKE @SearchTerm";
                }
