...
public partial class AddSchool : System.Web.UI.Page
{
    ...
    protected void drpClass_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        string examId = exam[0];

        string[] selectedClass = drpClass.Text.Split('|');
        int classId = int.Parse(selectedClass[0]);

        string classLevel = "";
        string query = @" Select Level from Classes where ClassID = @classId and SchoolId = @schoolId";
        SqlCommand command = new SqlCommand(query, appconSQL2);
        ...
        {
            Response.Redirect("ExamClassResults.aspx");
        }
        else if (classLevel == "Secondary - Senior")
        {
            Response.Redirect("ExamClassResults2.aspx");
        }
        else
        {
            Response.Redirect("ExamClassResults.aspx");
