...
public partial class AddSchool : System.Web.UI.Page
{
    ...
    {
        string query = "SELECT [SchoolId], [School_name], [School_email], [PhoneNumber], [School_address], " +
                       "[Slogan], [Logo], [Admin_name], [Admin_email], [Admin_phone], " +
                       "[Date_Registered], [RegisteredBy] FROM [AllSchools] WHERE [SchoolId] = @SchoolId";
        ...
        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["Myschools"].ConnectionString))
        {
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                ...
                if (reader.Read())
                {
                    ...
                    litSchoolEmail.Text = reader["School_email"].ToString();
                    litPhoneNumber.Text = reader["PhoneNumber"].ToString();
                    litSchoolAddress.Text = reader["School_address"].ToString();
                    litAdminName.Text = reader["Admin_name"].ToString();
                    litAdminEmail.Text = reader["Admin_email"].ToString();
                    litAdminPhone.Text = reader["Admin_phone"].ToString();
                    litDateRegistered.Text = Convert.ToDateTime(reader["Date_Registered"]).ToString("dd MMMM yyyy");
                    litRegisteredBy.Text = reader["RegisteredBy"].ToString();
